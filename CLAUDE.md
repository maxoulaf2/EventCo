...
- **Auth** : passwordless (code à 6 chiffres envoyé par email, valable 15 min, usage unique), cookie de session httpOnly
...
Architecture : un seul service **Render** (Docker) exécutant l'API, qui sert aussi le frontend buildé en statique (`wwwroot`, cf. `src/EventCo.Api/Dockerfile`) — same-origin, cohérent avec la décision déjà actée pour le dev (cf. `AuthController.cs`, `client/vite.config.ts`). Base de données **Neon** (PostgreSQL gratuit, ne s'auto-supprime pas). Stockage des photos de profil et des images d'événement **Neon Object Storage** (S3-compatible, 5 Go sur le plan gratuit, un bucket `public_read` par usage, lu directement par le navigateur ; sans configuration, fallback sur le disque local de l'API, effacé à chaque déploiement Render). Email **Brevo** (SMTP gratuit, 300/jour, pas de domaine à vérifier). `render.yaml` décrit le service (Blueprint Render).
...
4. Dans le dashboard Render du service, renseigner les variables d'environnement marquées `sync: false` dans `render.yaml` :
   ...
   - `Frontend__BaseUrl` : URL du service Render (ex. `https://eventco.onrender.com`), utilisée dans les liens d'invitation envoyés par email
   ...
   - `Storage__S3__ServiceUrl`/`Region`/`AccessKeyId`/`SecretAccessKey`/`Buckets__Avatars__Name`/`Buckets__EventImages__Name` : Neon Object Storage, cf. étape 10 ci-dessous
...
9. Pousser sur `main` (ou relancer le workflow) pour déclencher le premier déploiement, puis vérifier le parcours nominal en production (connexion par code, création d'événement, invitation avec envoi d'email réel).
10. **Neon Object Storage** (photos de profil, images d'événement) : dans le projet Neon, branche de production > onglet *Object Storage* :
    - créer deux buckets, `eventco-avatars` et `eventco-event-images` (noms libres), tous deux avec l'accès **`public_read`** (non modifiable ensuite par l'API S3 : `PutBucketAcl`/`PutBucketPolicy` ne sont pas supportés) ;
    - générer une clé d'accès S3 (valable pour les deux buckets) : `Access Key ID` (`nak_live_...`) et `Secret Access Key` (`nsk_live_...`), **affichées une seule fois** ;
    - relever l'endpoint de la branche (`https://br-<branche>.storage.c-1.<région>.aws.neon.tech`) et sa région AWS (ex. `eu-central-1`, sans le préfixe `aws-` de Neon) ;
    - renseigner dans Render `Storage__S3__ServiceUrl` (endpoint), `Storage__S3__Region`, `Storage__S3__AccessKeyId`, `Storage__S3__SecretAccessKey`, `Storage__S3__Buckets__Avatars__Name`, `Storage__S3__Buckets__EventImages__Name`, puis redéployer. Si l'un des deux noms de bucket manque alors que `ServiceUrl` est renseigné, l'API refuse de démarrer (message explicite dans les logs Render) ;
    - vérifier : ajout d'une photo depuis la modale « Mon compte », l'URL de l'image doit pointer vers `…aws.neon.tech/<bucket-avatars>/…` ; création d'un événement avec une image, qui doit pointer vers `…aws.neon.tech/<bucket-images>/…`. Au démarrage, un avertissement `Aucun bucket S3 configuré` dans les logs Render signale une configuration manquante.

    Les buckets sont rattachés à une **branche** Neon : une branche de preview en hérite (copy-on-write) sans toucher aux fichiers de production.
