...
- `Api.Tests` : tests d'intégration bout-en-bout écrits en **Gherkin/Reqnroll** (mêmes conventions que `Application.Tests` ci-dessus), via `WebApplicationFactory<Program>` + un vrai PostgreSQL démarré à la volée avec **Testcontainers** (`postgres:16-alpine`, cohérent avec `docker-compose.yml`).
  ...
  >
  > Piège rencontré : comparer une propriété du Value Object `Email` dans un prédicat LINQ traduit en SQL (`t.Email.Value == ...`) échoue avec le provider Npgsql (`InvalidOperationException`, traduction impossible), contrairement au provider InMemory qui évalue côté client — matérialiser (`ToListAsync()`) puis filtrer/comparer côté C# à la place.
  >
