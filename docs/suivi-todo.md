...
- [x] Deux natures d'article : « à prendre » (demande, créée sans attribution par le créateur/un co-organisateur uniquement) et « apporté » (contribution, automatiquement attribuée à son créateur, ni désattribuable ni réattribuable, annulable uniquement par son créateur) — nature stockée sur l'article ; un participant simple ne peut qu'ajouter ce qu'il apporte, un (co-)organisateur peut ajouter les deux (côté API ; dans l'UI, il ajoute toujours un article à prendre, qu'il s'assigne ensuite s'il veut le prendre)
  > Tâche identifiée en cours de route (rule 5, 2026-09-25), à la demande explicite du développeur (cf. `TODO.md`). Présentation de la liste d'articles inchangée. Migration des données existantes : article créé par un (co-)organisateur → « à prendre » ; article créé par un participant simple et attribué à lui-même → « apporté » ; tout autre article → « à prendre ».
  > Références visuelles du détail d'un événement (`apparence-detail-evenement.feature`) devenues obsolètes (placeholders « Ajouter un article à prendre… » côté organisateur, « Ce que j'apporte… » côté participant) et un nouveau scénario sans référence — à générer via le workflow GitHub Actions `update-visual-baselines` puis à valider/committer par le développeur.
...
- [x] Connexion par code à 6 chiffres reçu par email (remplace le lien magique), saisi directement sur la page « C'est parti » — y compris depuis un lien d'invitation — avec la même durée de validité (15 min) et un usage unique
  > Tâche identifiée en cours de route (rule 5, 2026-09-24), à la demande explicite du développeur, pour simplifier la connexion (plus besoin de basculer entre la boîte mail et l'appli, notamment en PWA).
  > Référence visuelle de la page de confirmation d'envoi (`apparence-pages-auth.feature`, scénario « Page de confirmation d'envoi ») devenue obsolète (champ de saisie du code ajouté, libellés « lien » → « code ») — à régénérer via le workflow GitHub Actions `update-visual-baselines` puis à valider/committer par le développeur. La variable d'environnement Render `MagicLink__VerificationUrlBase` n'est plus lue : peut être supprimée du dashboard Render.
...
- [x] CI : le workflow `update-visual-baselines` supprime les captures de référence orphelines (scénario retiré ou renommé) au lieu de les re-committer
  > Tâche identifiée en cours de route (rule 5, 2026-09-24), à la demande explicite du développeur. `--update-snapshots` ne supprime jamais une capture orpheline, et le job de régénération partait du checkout (anciennes captures incluses) : elles étaient remontées dans l'artefact, ce qui rendait inopérant le `rm -rf` du job d'ouverture de PR. Orphelines actuellement versionnées (7 fichiers : « onglet des tâches assignées », « onglet des tâches faites », « invitation en attente » mobile) — supprimées au prochain lancement du workflow.
- [x] Versionnage du build (SHA git) partagé par l'API et le frontend : rechargement silencieux du frontend (mise à jour du service worker PWA incluse) quand il détecte que l'API tourne dans une autre version
  > Tâche identifiée en cours de route (rule 5, 2026-09-25), à la demande explicite du développeur : après un déploiement, un onglet resté ouvert ou la PWA installée continuent d'exécuter l'ancien bundle (servi depuis le cache du service worker).
