...
Fonctionnalité: Demande de code de connexion via l'API
  ...
  Scénario: Email valide
    Quand j'envoie une requête POST à "/api/auth/request-code" avec l'email "api-test@example.com"
  ...
  Scénario: Trop de demandes pour le même email
    Quand j'envoie 6 requêtes POST à "/api/auth/request-code" avec l'email "api-spam@example.com"
    Alors la réponse a le statut 429

  Scénario: Email invalide
    Quand j'envoie une requête POST à "/api/auth/request-code" avec l'email "pas-un-email"
