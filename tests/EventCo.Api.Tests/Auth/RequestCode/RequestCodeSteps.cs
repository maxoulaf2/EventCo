...
namespace EventCo.Api.Tests.Auth.RequestCode;

[Binding]
public sealed class RequestCodeSteps
{
    ...
    public async Task QuandJenvoieUneRequetePostAAvecLemail(string path, string email)
    {
        ...
        _response = await client.PostAsJsonAsync(path, new RequestLoginCodeRequest(email));
    }
    ...
    public async Task AlorsUnCodeDeConnexionEstPersisteEnBasePour(string email)
    {
        ...
        var tokens = await dbContext.LoginCodes.ToListAsync();
